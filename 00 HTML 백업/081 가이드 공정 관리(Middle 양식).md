...
**작성일**: 2026-01-27
**버전**: v4.28 ([필터링 Observer 누적 해결] 안정적 ID 생성, 필터 렌더 전 정리, 총 76개 항목)
**대상 파일**: `A00 공정 관리(Middle 양식).html` | `00 공정 관리(Middle 양식).html` | `01 공정 및 정산.html` | `01 공정 관리(Middle 양식).html` | `작업 유형별 현황_(기숙사 및 사택).html` ❌ 3차 대기
...
4. **[v1.7] 선제적 정리 실패 / 복귀 지연 발생**
   ...
   - Observer 누적 > 10개 (`ObserverPool.getAllStats().total` 확인)

5. **[v1.8] IntegrityEnhancementModule 로드 실패** ⚠️ 2026-01-26 발견
...
| **[v1.7]** Observer 10+개 누적 | 주기적 정리 1분/10개 제거 | `ObserverPool.getAllStats().total` 확인 |
...
// Observer 누적 확인 (10개 이하여야 정상)
const totalObservers = window.ObserverPool?.getAllStats?.()?.total || 0;
console.log('Total Observers (should be ≤10):', totalObservers, totalObservers <= 10 ? '✅' : '⚠️');
...
### v1.5/v1.6/v1.6.3/v1.7/v1.8 롤백 시 태그 검색 순서

1. **v4.28만 롤백**: `[v4.28]` 또는 `[FIX] v4.28` 검색 후 해당 블록을 이전 버전으로 복원
